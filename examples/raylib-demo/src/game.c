...
#define MAX_SHADOWS \
    MAX_BOXES *     \
        3  // MAX_BOXES *3. Each box can cast up to two shadow volumes for the
           // edges it is away from, and one for the box itself
...
// It takes the edge and projects it back by the light radius and turns it into
// a quad
void ComputeShadowVolumeForEdge(int slot, Vector2 sp, Vector2 ep) {
    ...
    Vector2 spVector =
        Vector2Normalize(Vector2Subtract(sp, lights[slot].position));
    Vector2 spProjection = Vector2Add(sp, Vector2Scale(spVector, extension));

    Vector2 epVector =
        Vector2Normalize(Vector2Subtract(ep, lights[slot].position));
...
void DrawLightMask(int slot) {
    ...
    if (lights[slot].valid)
        DrawCircleGradient(
            (int)lights[slot].position.x, (int)lights[slot].position.y,
...
bool UpdateLight(int slot, Rectangle *boxes, int count) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        // Check the edges that are on the same side we are, and cast shadow
        // volumes out from them
        ...
        // The box itself
        lights[slot].shadows[lights[slot].shadowCount].vertices[0] =
            (Vector2){boxes[i].x, boxes[i].y};
        lights[slot].shadows[lights[slot].shadowCount].vertices[1] =
            (Vector2){boxes[i].x, boxes[i].y + boxes[i].height};
        lights[slot].shadows[lights[slot].shadowCount].vertices[2] = (Vector2){
            boxes[i].x + boxes[i].width, boxes[i].y + boxes[i].height};
        lights[slot].shadows[lights[slot].shadowCount].vertices[3] =
            (Vector2){boxes[i].x + boxes[i].width, boxes[i].y};
        lights[slot].shadowCount++;
    }
...
void SetupBoxes(Rectangle *boxes, int *count) {
    ...
    for (int i = 5; i < MAX_BOXES; i++) {
        boxes[i] = (Rectangle){(float)GetRandomValue(0, GetScreenWidth()),
...
void Game_run(void) {
    ...
    while (!WindowShouldClose())  // Detect window close button or ESC key
    {
        ...
        if (IsMouseButtonPressed(MOUSE_BUTTON_RIGHT) &&
            ...
            SetupLight(nextLight, GetMousePosition().x, GetMousePosition().y,
                       200);
            nextLight++;
        }
        ...
        // Update the lights and keep track if any were dirty so we know if we
        // need to update the master light mask
        ...
        if (dirtyLights) {
            ...
            for (int i = 0; i < MAX_LIGHTS; i++) {
                if (lights[i].active)
                    DrawTextureRec(lights[i].mask.texture,
        ...
        DrawTextureRec(lightMask.texture,
                       (Rectangle){0, 0, (float)GetScreenWidth(),
                                   -(float)GetScreenHeight()},
                       Vector2Zero(),
                       ColorAlpha(WHITE, showLines ? 0.75f : 1.0f));

        // Draw the lights
        for (int i = 0; i < MAX_LIGHTS; i++) {
            if (lights[i].active)
                DrawCircle((int)lights[i].position.x, (int)lights[i].position.y,
                           10, (i == 0) ? YELLOW : WHITE);
        }
        ...
        if (showLines) {
            ...
            for (int b = 0; b < boxCount; b++) {
                ...
                DrawRectangleLines((int)boxes[b].x, (int)boxes[b].y,
                                   (int)boxes[b].width, (int)boxes[b].height,
                                   DARKBLUE);
            }
